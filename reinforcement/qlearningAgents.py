...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        """
        features = self.featExtractor.getFeatures(state, action)  # returns util.Counter
        qVal = 0.0
        for f, value in features.items():
            qVal += self.weights[f] * value
        return qVal
    ...
    def update(self, state, action, nextState, reward):
        ...
        if legalActions:  
            nextQ = max([self.getQValue(nextState, a) for a in legalActions])

        difference = (reward + self.discount * nextQ) - currQ
